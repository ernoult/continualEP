...
def train(net, train_loader, epoch, method): 
    if not hasattr(net, 'C_tab'):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):            
            ...
            if method == 'BPTT':    
                ...
                with torch.no_grad():
                    s, inds = net.forward(data, s, inds)
                    pred = s[0].data.max(1, keepdim=True)[1]
                    loss = (1/(2*s[0].size(0)))*criterion(s[0], targets)
                    ###################################* EQPROP *############################################
                    seq = []
                    ...
                    for i in range(len(s)):       
                        ...
                        if inds[i] is not None:
                            ...
                            indseq.append(None)
                    s, inds = net.forward(data, s, inds, seq, indseq, target = targets, beta = net.beta, method = 'nograd')    
                    net.updateWeights(net.beta, data, s, inds, seq, indseq) 
                    #########################################################################################
        ...
        #print(100*(len(train_loader.dataset)- correct.item())/ len(train_loader.dataset))
    ...
    if not hasattr(net, 'C_tab'):
        ...
        with torch.no_grad():
            for batch_idx, (data, targets) in enumerate(test_loader): 
                ...
                s, inds = net.forward(data, s, inds, method = 'nograd')
...
def compute_nSdSDT(net, data, target): 
    ...
    elif hasattr(net, 'C_tab') & hasattr(net, 'size_convpool_tab'):
        beta = net.beta
        batch_size_temp = data.size(0)
    ...
    elif hasattr(net, 'C_tab') & (not hasattr(net, 'size_convpool_tab')):
        beta = net.beta
        batch_size_temp = data.size(0)
        s = net.initHidden(batch_size_temp)    
        if net.cuda: 
...
def receipe(net, train_loader, N_trials):
    ...
    if hasattr(net, 'conv'):        
        counter_sign_Theta_fc = np.zeros((N_trials, len(net.fc)))
        counter_sign_Theta_conv = np.zeros((N_trials, len(net.conv)))
        counter_zero_Theta_fc = np.zeros((N_trials, len(net.fc)))
        counter_zero_Theta_conv = np.zeros((N_trials, len(net.conv)))
        ...
        for n in range(N_trials):
            ...
            nS, dS, DT_conv, DT_fc, _ = computedSnSDT(net, data, targets)

            #Check NT computation		       
            NT_conv, NT_fc = computeNT(net, data, targets, wholeProcess = False)
            #print('NT computed')        
            ...
            for i in range(len(NT_fc)):
                ...
                counter_sign_Theta_fc[n, i] = counter_temp
                counter_zero_Theta_fc[n, i] = counter_temp_2
                #print(counter_sign_Theta_fc[i])
                #print(counter_zero_Theta_fc[i])
            ...
            for i in range(len(NT_conv)):
                ...
                counter_sign_Theta_conv[n, i] = counter_temp
                counter_zero_Theta_conv[n, i] = counter_temp_2
                #print(counter_sign_Theta_conv[i])
                #print(counter_zero_Theta_conv[i])
        ...
        for i in range(len(NT_fc)):
            print('average fc layer {}: {:.1f} +- {:.1f}%  (same sign, total), i.e. {:.1f} +- {:.1f}%  (stricly non zero), {:.1f} +- {:.1f}%  (both zero)'.format(i, 
                    counter_sign_Theta_fc[:, i].mean() + counter_zero_Theta_fc[:, i].mean(), 
                    counter_sign_Theta_fc[:, i].std() + counter_zero_Theta_fc[:, i].std(), 
                    counter_sign_Theta_fc[:, i].mean(), 
                    counter_sign_Theta_fc[:, i].std(), 
                    counter_zero_Theta_fc[:, i].mean(),
                    counter_zero_Theta_fc[:, i].std()))

        for i in range(len(NT_conv)):
            print('average conv layer {}: {:.1f} +- {:.1f}%  (same sign, total), i.e. {:.1f} +- {:.1f}%  (stricly non zero), {:.1f} +- {:.1f}%  (both zero)'.format(i, counter_sign_Theta_conv[:, i].mean() + counter_zero_Theta_conv[:, i].mean(), 
                counter_sign_Theta_conv[:, i].std() + counter_zero_Theta_conv[:, i].std(), 
                counter_sign_Theta_conv[:, i].mean(), 
                counter_sign_Theta_conv[:, i].std(),
                counter_zero_Theta_conv[:, i].mean(),
                counter_zero_Theta_conv[:, i].std()))
        print('***********************************************')
        print('done')
        ...
        for n in range(N_trials):
            ...
            #Check dS, nS, DT computation
            nS, dS, DT, _ = computedSnSDT(net, data, targets)
